...
class DiffusionInferer(Inferer):
    ...
    def __init__(self, scheduler: nn.Module, style_stats=False) -> None:
        Inferer.__init__(self)
    ...
    def __call__(
        ...
        noisy_image, *rest = self.scheduler.add_noise(original_samples=inputs, noise=noise, timesteps=timesteps)

        if len(noisy_image.shape) != 5:
        ...
        if self.style_stats:
            prediction, mean, std = diffusion_model(x=noisy_image, timesteps=timesteps, context=condition, 
                                                    class_labels=class_label,ema_mean=ema_mean, ema_std=ema_std)
        else:
        ...
        if return_snr:
            snr = rest[0]
            if self.style_stats:
                return prediction, noisy_image, snr, mean, std
            else:
                return prediction, noisy_image, snr
        else:
