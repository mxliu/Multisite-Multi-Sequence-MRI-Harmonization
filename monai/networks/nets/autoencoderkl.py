...
class AutoencoderKL(nn.Module):
    ...
    def encode(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:  ### use this next !
        h = self.encoder(x) if not self.use_checkpoint else \
            torch.utils.checkpoint.checkpoint(self.encoder, x, use_reentrant=False)
        z_mu = self.quant_conv_mu(h)

        # Predict log-variance directly (preferred)
        z_log_var = self.quant_conv_log_sigma(h)   # raw log-variance
        z_log_var = torch.clamp(z_log_var, min=-30.0, max=20.0)
        z_sigma = torch.exp(0.5 * z_log_var) + 1e-8
    ...
    #     # Modified stability additions
    #     z_log_var = self.quant_conv_log_sigma(h)
        
    #     # 1. Use softplus for smooth constraint
    #     z_log_var = F.softplus(z_log_var)  # Output > 0
        
    #     # 2. Apply conservative clamping (optional but recommended)
    #     z_log_var = torch.clamp(z_log_var, min=-15.0, max=15.0)  # Tighter than before
        
    #     # 3. Calculate sigma with stability epsilon
    #     z_sigma = torch.exp(z_log_var / 2) + 1e-8  # Critical addition
    ...
    def encode_stage_2_inputs_ns(self, x: torch.Tensor) -> torch.Tensor:
        z_mu, z_sigma = self.encode(x)
        return z_mu
